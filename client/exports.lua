local newExports = function (funcName, func)
    -- Export for LegacyFuel compatibility
    AddEventHandler(('__cfx_export_LegacyFuel_%s'):format(funcName), function(setCB)
        setCB(func)
    end)
    -- Export for qb-fuel compatibility
    AddEventHandler(('__cfx_export_qb-fuel_%s'):format(funcName), function(setCB)
        setCB(func)
    end)
    -- Export for the current resource (sg-fuel)
...
local SetFuel = function (vehicle, fuel)
    ...
    if Config.SyncFuelBetweenPlayers then Entity(vehicle).state:set('sg-fuel', fuel + 0.0, true) end
end
...
if Config.SyncFuelBetweenPlayers then
    ...
    AddStateBagChangeHandler("sg-fuel", nil, function(bagName, _, value)
        ...
        CreateThread(function ()
            while DoesEntityExist(vehicle) and GetPedInVehicleSeat(vehicle, -1) == playerPed do
                if IsVehicleEngineOn(vehicle) then
                    Entity(vehicle).state:set('sg-fuel', GetVehicleFuelLevel(vehicle) + 0.0, true)
                end
        ...
        CreateThread(function ()
            while DoesEntityExist(vehicle) and GetPedInVehicleSeat(vehicle, -1) == playerPed do
                if IsVehicleEngineOn(vehicle) then
                    ...
                    Entity(vehicle).state:set('sg-fuel', fuel + 0.0, true)
                end
                Wait(Config.FuelSyncTime*1000)
            end
        end)
